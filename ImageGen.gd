...
# Link to the nodes in your scene
@onready var http_request = $APIRequest
@onready var prompt_input = $PromptInput
@onready var generate_button = $Sprite/GenerateButton
...
@onready var health_bar = $Sprite/StatList/HealthBar
@onready var damage_bar = $Sprite/StatList/DamageBar
@onready var speed_bar = $Sprite/StatList/SpeedBar
...
# Use placeholders like {descriptor} and {subject} that we can easily replace.
...
# This function runs when you click the GenerateButton
...
func send_api_request():
	...
	print(sprite_container.scale.x)
	...
	var image_prompt = "A full body shot of a "
	image_prompt += description[0] + " " + description[1]
	image_prompt += ", orthographic view, anime style, pure green background"
...
# This function runs automatically when the server sends a response
...
func update_character(stats):
	...
	tween.tween_property($Camera2D, "zoom", Vector2(80 / stats.size, 80 / stats.size) / 2, 3.0)
	tween.tween_property(sprite_container, "scale", Vector2(stats.size, stats.size) / 100, 3.0)
